...
const FundTransfer = () => {
  ...
  const handleP2PTransfer = async () => {
    if(!formData.amount || !formData.otp || !formData.type || !formData.tnxPass){
      return Swal.fire({
        icon: "error",
        title: "Error",
    ...
    try {
      ...
      console.log(payload)
      const response = await makeP2PTransfer(payload);
      if (response?.success) {
        Swal.fire({
          icon: "success",
          title: "Success",
      ...
      console.log("error in p2p transfer", error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error?.response?.data?.message || "Failed to transfer",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleGetOtp = async () => {
    const res = await sendOtptoUserP2p();
    ...
    <>
      ...
      <div className="min-h-screen text-white mt-5">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold mt-4">P2P Transfer</h1>
        </div>
          ...
          {searchedUser && (
            <div className="bg-[#111] border !border-gray-700 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 transition-all duration-200">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-200 !hover:border-gray-400 transition-all">
                  <img
          ...
          {searchedUser && (
            <>
              <ReusableForm
                ...
                icon={Wallet}
                options={[
                  { value: "currentEarnings", label: "Main Wallet" },
              ...
              />
               ...
               <ReusableForm
                ...
                disabled={
                  formData.type === "currentEarnings"
                    ? userInfo?.currentEarnings < formData.amount 
                    : userInfo?.packageWallet < formData.amount || !formData.amount || !formData.type || !formData.tnxPass
                }
              />
          
              <div className="flex justify-end pt-4">
                <button
                  disabled={loading  ||  formData.type === "currentEarnings"  
                    ? userInfo?.currentEarnings < formData.amount 
                    : userInfo?.packageWallet < formData.amount 
                  }
                  onClick={handleP2PTransfer}
                  className="bg-[var(--cyan-active)] hover:scale-105 text-white font-bold px-10 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl text-xl disabled:cursor-not-allowed"
                >
                  Submit
                </button>
              </div>
