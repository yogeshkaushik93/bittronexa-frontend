...
const RankAndReward = () => {
  ...
  const columns = [
    ...
    { label: "Status", key: "status", render: (val, row, index) => {
      if (row.status === "notAchieved") {
        return <span className="text-yellow-500">{row.status}</span>;
      } else {
        return <span className="text-green-500">{row.status}</span>;
      }
    } },
