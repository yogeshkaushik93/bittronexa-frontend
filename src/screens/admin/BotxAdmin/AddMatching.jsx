...
import { addMatchingByAdmin, getUserByName } from "../../../api/admin-api";
import {  User } from "lucide-react";
import Swal from "sweetalert2";
import ReusableForm from "../../../components/ui/ReusableForm";
import { getUerDetails } from "../../../api/admin-api";
...
const AddMatching = () => {
 ...
 const [weekLegCount, setweekLegCount] = useState("")
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await addMatchingByAdmin(payload);
            if (res?.success) {
                Swal.fire({
                    icon: "success",
                    title: "Add Matching Successful",
                    text: res.message || "Add matching has been successfully added!",
                    confirmButtonColor: "#3085d6",
                });
                setCount("");
            }
        } catch (error) {
            console.error("add matching error", error);
            Swal.fire({
                icon: "error",
        ...
        <div className="min-h-screen  text-white p-10 text-[1.4rem] leading-relaxed">
            <div className="max-w-7xl mx-auto">
                ...
                {loading ? (
                            ...
                            <div className="flex items-center gap-8 mb-10">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-4 mt-4">
                                        <div
                                            className={`flex items-center gap-3 px-5 py-2 rounded-full ${userData.isVerified
                                                ? "bg-green-500/20 text-green-400"
                                                : "bg-red-500/20 text-red-400"
                                                }`}
                                        >
                                            <div
                                                className={`w-3 h-3 rounded-full ${userData.isVerified ? "bg-green-500" : "bg-red-500"
                                                    }`}
                                            ></div>
                 ...
                 <div className="px-5 py-4 rounded-full bg-blue-500/20 text-blue-400 text-3xl font-medium">
                                           WeekLegCount : {weekLegCount?.weakLegCount}
                                        </div>
                ...
                <div className="backdrop-blur-xl  border-b border-white/10 rounded-3xl p-10 shadow-2xl">
                    ...
                    <div className="backdrop-blur-sm bg-white/5 rounded-2xl p-6 border border-white/10 mb-8">
                        <ReusableForm
                            ...
                            placeholder="Enter Matching Count"
                            icon={User}
                        />
                    </div>
